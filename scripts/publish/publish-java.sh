...
ensure_cmd mvn "Install Apache Maven"
mvn -q test

if [[ -n "${OSSRH_USERNAME:-}" && -n "${OSSRH_PASSWORD:-}" ]]; then
  mvn deploy
else
  echo "[dry-run] mvn deploy (set OSSRH_USERNAME and OSSRH_PASSWORD to publish)"
  mvn -q package
  echo "Would run: mvn deploy"
fi
